...
def _conditions_satisfied(
    conditions: list[str], stdout: list[str], match: str
) -> tuple[bool, list[str]]:
    """Return wait_for status and conditions that are still unsatisfied."""
    failed = [
        condition
        for condition in conditions
        if not _condition_matches(condition, stdout)
    ]
    if match == "any":
...
def main() -> None:
    ...
    try:
        commands = validate_command_entries(module.params["commands"])
    except ValueError as exc:
        module.fail_json(
            msg="invalid command input", error=str(exc), changed=False, commands=[]
        )
        return
    ...
    if retries <= 0 or interval < 0:
        module.fail_json(
            msg="invalid wait parameters",
            changed=False,
            commands=redact_value(commands),
            error="retries must be greater than zero and interval must be non-negative",
        )
        return
    ...
    if unsafe_commands and not module.params.get("unsafe_allow_mutating_commands"):
        module.fail_json(
            ...
            commands=redact_value(unsafe_commands),
        )
        return
    if unsafe_commands:
        module.warn(
            "unsafe_allow_mutating_commands enabled for guarded commands: {0}".format(
                ", ".join(redact_value(unsafe_commands))
            )
        )
    ...
    try:
        for attempt in range(retries):
            ...
            if not wait_for:
                break
            satisfied, failed_conditions = _conditions_satisfied(
                wait_for, stdout, module.params.get("match")
            )
            if satisfied:
                failed_conditions = []
                break
        ...
        module.fail_json(
            ...
            context=getattr(exc, "context", "command"),
        )
        return
        ...
        module.fail_json(
            msg="command execution failed",
            commands=redact_value(commands),
            error=redact_value(to_text(exc)),
            context="command",
        )
