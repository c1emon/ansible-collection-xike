...
class ActionModule(NormalActionModule):
    ...
    def run(self, tmp: Any = None, task_vars: dict[str, Any] | None = None) -> dict[str, Any]:
        """Inject the bundled TextFSM templates before running the action."""
        ...
        results: dict[str, Any] = super(ActionModule, self).run(tmp=tmp, task_vars=task_vars)
        return results

    def _load_textfsm_template(self, template_name: str) -> str:
        """Load a bundled TextFSM template from the collection."""
