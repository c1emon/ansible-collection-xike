...
from ansible_collections.c1emon.xikeos.plugins.action import (
    xikeos_vlans as action_vlans_module,
)
...
from ansible_collections.c1emon.xikeos.plugins.module_utils.network.xikeos import (
    xikeos as network_utils,
)
from ansible_collections.c1emon.xikeos.plugins.module_utils.network.xikeos import (
    errors as xikeos_errors,
)
from ansible_collections.c1emon.xikeos.plugins.module_utils.network.xikeos import (
    safety as safety_utils,
)
from ansible_collections.c1emon.xikeos.plugins.module_utils.facts import (
    ospfv2 as ospfv2_facts_module,
)
...
def test_cliconf_get_config_edit_config_and_capabilities():
    ...
    plugin.send_command = Mock(
        side_effect=["running-config", "term", "vlan", "name", "end"]
    )

    assert (
        plugin.get_config(source="startup", flags=["all", "brief"], format="text")
        == "running-config"
    )
    ...
    result = json.loads(
        plugin.edit_config(
            candidate=[{"command": "vlan 100"}, "! ignored", "end", "name DATA"]
        )
    )
    assert result == {
        "diff": "",
        "request": ["vlan 100", "name DATA"],
        "response": ["vlan", "name"],
    }
    ...
    with patch.object(
        ...
        return_value=json.dumps(
            {"rpc": [], "device_operations": {}, "format": [], "network_api": ""}
        ),
    ):
        capabilities = json.loads(plugin.get_capabilities())
...
@pytest.mark.parametrize(
    ...
    [
        ("Hostname: core-switch", "core-switch"),
        ("System name: edge-switch", "edge-switch"),
    ],
)
def test_cliconf_get_device_info_parses_hostname_variants(
    version_line, expected_hostname
):
    plugin = Cliconf.__new__(Cliconf)
    plugin.get = Mock(
        return_value="\n".join(
            [version_line, "Software version: 1.2.3", "Model: X1000"]
        )
    )
...
def test_network_load_config_uses_candidate_and_propagates_typeerror():
    module = Mock()
    connection = Mock()
    connection.edit_config.side_effect = [
        TypeError("candidate is required"),
        {"response": ["ok"]},
    ]
...
def test_network_helpers_raise_typed_errors_on_connection_failure():
    module = Mock()
    connection = Mock()
    connection.run_commands.side_effect = network_utils.ConnectionError(
        "transport lost"
    )
...
def test_xikeos_error_string_and_detail_fields():
    err = xikeos_errors.XikeOSConfigError(
        "boom", commands=["vlan 10"], detail="details", context="config"
    )
...
def test_network_load_config_decodes_json_string_response():
    module = Mock()
    connection = Mock()
    connection.edit_config.return_value = json.dumps(
        {"response": ["ok"], "request": ["vlan 10"]}
    )

    with patch.object(network_utils, "get_connection", return_value=connection):
        assert network_utils.load_config(module, ["vlan 10"]) == {
            "response": ["ok"],
            "request": ["vlan 10"],
        }
...
def test_network_get_config_uses_flags_and_returns_text():
    ...
    with patch.object(network_utils, "get_connection", return_value=connection):
        assert (
            network_utils.get_config(module, flags=["all", "brief"]) == "running-config"
        )

    connection.get_config.assert_called_once_with(
        source="running", flags=["all", "brief"], format=None
    )
...
def test_xikeos_command_stdout_and_lines():
    ...
    with (
        patch.object(command_module, "AnsibleModule", return_value=module),
        patch.object(
...
def test_xikeos_command_blocks_mutating_commands_by_default():
    ...
    with (
        patch.object(command_module, "AnsibleModule", return_value=module),
        patch.object(command_module, "run_commands") as run_mock,
    ):
        with pytest.raises(ExitJson):
            command_module.main()
...
def test_redaction_preserves_context_and_hides_secret_values():
    output = "\n".join(
        [
            "username admin privilege 15 password supersecret",
            "snmp-server community public RO",
            "radius-server host 192.0.2.10 key radius-secret",
    ...
    assert "radius-secret" not in redacted
    assert "tacacs-secret" not in redacted
    assert safety_utils.REDACTION_MARKER in redacted
    assert "interface ethernet 1/0/1" in redacted


def test_command_and_config_reject_compound_input_before_device_access_and_sanitize_results():
    command = _fake_module(
        {"commands": ["show version\nreload"], "retries": 1, "interval": 0}
    )
    command.fail_json.side_effect = ExitJson
    with (
        patch.object(command_module, "AnsibleModule", return_value=command),
        patch.object(command_module, "run_commands") as run_mock,
    ):
        with pytest.raises(ExitJson):
            command_module.main()
    run_mock.assert_not_called()
    assert command.fail_json.call_args.kwargs["msg"] == "invalid command input"

    invalid_wait = _fake_module(
        {"commands": ["show version"], "retries": 0, "interval": 0}
    )
    invalid_wait.fail_json.side_effect = ExitJson
    with (
        patch.object(command_module, "AnsibleModule", return_value=invalid_wait),
        patch.object(command_module, "run_commands") as run_mock,
    ):
        with pytest.raises(ExitJson):
            command_module.main()
    run_mock.assert_not_called()
    assert invalid_wait.fail_json.call_args.kwargs["msg"] == "invalid wait parameters"

    config = _fake_module({"lines": ["username admin password config-secret"]})
    with (
        patch.object(config_module, "AnsibleModule", return_value=config),
        patch.object(
            config_module,
            "load_config",
            return_value={"response": {"password": "response-secret"}},
        ),
    ):
        with pytest.raises(ExitJson):
            config_module.main()
    payload = config.exit_json.call_args.kwargs
    assert "config-secret" not in repr(payload)
    assert "response-secret" not in repr(payload)
...
def test_xikeos_facts_minimum_device_facts():
    ...
    with (
        patch.object(facts_module, "AnsibleModule", return_value=module),
        patch.object(facts_module, "run_commands", return_value=[version_output]),
    ):
        with pytest.raises(ExitJson):
            facts_module.main()
...
def test_xikeos_facts_gathers_resource_facts_under_ansible_network_resources():
    module = _fake_module(
        {"gather_subset": ["min"], "gather_network_resources": ["vlans", "interfaces"]}
    )
    with (
        patch.object(facts_module, "AnsibleModule", return_value=module),
        patch.object(facts_module, "run_commands", return_value=["Hostname: core"]),
        patch.object(
            facts_module,
            "gather_vlans",
            return_value=[{"vlan_id": 10, "name": "DATA", "state": "active"}],
        ),
        patch.object(
            facts_module,
            "gather_interfaces",
            return_value={"ethernet 1/0/1": {"description": "uplink", "enabled": True}},
        ),
    ):
        with pytest.raises(ExitJson):
            facts_module.main()
    ...
    assert facts["ansible_network_resources"]["vlans"] == [
...
def test_xikeos_facts_reports_context_for_typed_gather_failures():
    module = _fake_module(
        {"gather_subset": ["min"], "gather_network_resources": ["vlans"]}
    )
    module.fail_json.side_effect = ExitJson
    ...
    with (
        ...
        patch.object(
            facts_module,
            "run_commands",
            side_effect=xikeos_errors.XikeOSCommandError(
                "show version failed", commands=["show version"], detail="lost"
            ),
        ),
        patch.object(
            facts_module,
            "gather_vlans",
            side_effect=xikeos_errors.XikeOSFactsError(
                "vlan gather failed", detail="bad", context="vlans"
            ),
        ),
    ):
        with pytest.raises(ExitJson):
...
def test_xikeos_facts_golden_sks8300_resource_shapes_are_config_compatible():
    module = _fake_module(
        {
            "gather_subset": ["min"],
            "gather_network_resources": ["vlans", "l2_interfaces", "l3_interfaces"],
        }
    )
    version_output = (
        "Hostname: sks8300-a\nSoftware version: V300SP10240912\nModel: SKS8300"
    ...
    l3_facts = {
        "vlan-interface 10": {
            "ipv4": [{"address": "192.0.2.1", "subnet_mask": "255.255.255.0"}],
            "ipv6": [],
        }
    }
    ...
    with (
        patch.object(facts_module, "AnsibleModule", return_value=module),
        patch.object(facts_module, "run_commands", return_value=[version_output]),
        patch.object(facts_module, "gather_vlans", return_value=vlan_facts),
        patch.object(facts_module, "gather_l2_interfaces", return_value=l2_facts),
        patch.object(facts_module, "gather_l3_interfaces", return_value=l3_facts),
    ):
        with pytest.raises(ExitJson):
            facts_module.main()

    resources = module.exit_json.call_args.kwargs["ansible_facts"][
        "ansible_network_resources"
    ]
    assert resources["vlans"] == [{"vlan_id": 10, "name": "servers", "state": "active"}]
    assert resources["l2_interfaces"] == [
        {"name": "ethernet 1/0/1", "mode": "access", "access_vlan": 10, "pvid": 10}
    ]
...
def test_xikeos_config_check_mode_and_save_flow():
    ...
    with (
        patch.object(config_module, "AnsibleModule", return_value=check_module),
        patch.object(config_module, "load_config") as load_mock,
    ...
    with (
        patch.object(config_module, "AnsibleModule", return_value=module),
        patch.object(
    ...
    run_mock.assert_called_once_with(
        module, [config_module.SAVE_COMMAND], check_rc=True
    )
...
def test_xikeos_config_apply_failure_stops_before_save():
    ...
    with (
        ...
        patch.object(
            config_module,
            "load_config",
            side_effect=xikeos_errors.XikeOSConfigError(
                "apply failed", commands=["vlan 10"], detail="down"
            ),
        ),
        patch.object(config_module, "run_commands") as run_mock,
    ):
...
def test_xikeos_config_save_failure_reports_applied_context():
    ...
    with (
        patch.object(config_module, "AnsibleModule", return_value=module),
        patch.object(config_module, "load_config", return_value={"response": ["ok"]}),
        patch.object(
            config_module,
            "run_commands",
            side_effect=xikeos_errors.XikeOSCommandError(
                "save failed", commands=[config_module.SAVE_COMMAND], detail="disk full"
            ),
        ),
    ):
        with pytest.raises(ExitJson):
    ...
    assert module.fail_json.call_args.kwargs["changed"] is True
    assert module.fail_json.call_args.kwargs["saved"] is False
    assert module.fail_json.call_args.kwargs["commands"] == [
        "vlan 10",
        config_module.SAVE_COMMAND,
    ]
...
def test_xikeos_vlans_normalize_and_lifecycle():
    assert vlans_module._normalize_vlan(
        {"vlan_id": "100", "status": "suspend", "ports": ["e0/0/1"]}
    ) == {
        "vlan_id": 100,
        "name": "",
        "state": "suspend",
    ...
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=unchanged),
        patch.object(vlans_module, "gather_vlans", return_value=current) as gather_mock,
    ...
    omitted_name = _fake_module(
        {"config": [{"vlan_id": 100, "state": "active"}], "state": "merged"}
    )
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=omitted_name),
        patch.object(vlans_module, "gather_vlans", return_value=current),
        patch.object(vlans_module, "load_config") as load_mock,
    ):
        with pytest.raises(ExitJson):
            vlans_module.main()
    ...
    changed = _fake_module(
        {
            "config": [{"vlan_id": 100, "name": "VOICE", "state": "active"}],
            "state": "merged",
        }
    )
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=changed),
        patch.object(vlans_module, "gather_vlans", return_value=current),
        patch.object(vlans_module, "load_config") as load_mock,
    ):
        with pytest.raises(ExitJson):
    ...
    assert changed.exit_json.call_args.kwargs["commands"] == [
        "vlan 100",
        "description VOICE",
        "exit",
    ]
    assert changed.exit_json.call_args.kwargs["after"] == [
        {"vlan_id": 100, "name": "VOICE", "state": "active"}
    ]
    load_mock.assert_called_once_with(
        changed, ["vlan 100", "description VOICE", "exit"]
    )
    ...
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=deleted),
        patch.object(vlans_module, "gather_vlans", return_value=before_deleted),
        patch.object(vlans_module, "load_config") as load_mock,
    ):
        with pytest.raises(ExitJson):
            vlans_module.main()
    ...
    assert deleted.exit_json.call_args.kwargs["after"] == [
        {"vlan_id": 200, "name": "VOICE", "state": "active"}
    ]
    load_mock.assert_called_once_with(deleted, ["no vlan 100"])

    check_mode = _fake_module(
        {"config": [{"vlan_id": 300, "name": "NEW"}], "state": "merged"},
        check_mode=True,
    )
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=check_mode),
        patch.object(vlans_module, "gather_vlans", return_value=[]),
        patch.object(vlans_module, "load_config") as load_mock,
    ):
        with pytest.raises(ExitJson):
            vlans_module.main()
    ...
    assert check_mode.exit_json.call_args.kwargs["commands"] == [
        "vlan 300",
        "description NEW",
        "exit",
    ]
    ...
    gathered_vlans = [
        {
            ...
            "name": "dev",
            "state": "active",
            "ports": [{"name": "Ethernet1/0/3", "tagged": True}],
        }
    ]
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=gathered),
        patch.object(vlans_module, "gather_vlans", return_value=gathered_vlans),
        patch.object(vlans_module, "load_config") as load_mock,
    ):
        with pytest.raises(ExitJson):
            vlans_module.main()

    assert gathered.exit_json.call_args.kwargs == {
        "changed": False,
        "gathered": gathered_vlans,
    }
    load_mock.assert_not_called()
...
"""
    ...
    with patch.object(
        vlans_module, "run_commands", side_effect=Exception("connection lost")
    ):
...
def test_xikeos_vlans_fails_explicitly_for_unsupported_mutating_edge_cases():
    suspended = _fake_module(
        {"config": [{"vlan_id": 100, "state": "suspend"}], "state": "merged"}
    )
    suspended.fail_json.side_effect = ExitJson
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=suspended),
    ...
    with (
        patch.object(vlans_module, "AnsibleModule", return_value=default_delete),
        pytest.raises(ExitJson),
    ):
        vlans_module.main()
...
def test_static_routes_lifecycle_uses_network_apply_and_check_mode_computes_diff():
    ...
    with (
        patch.object(static_routes_module, "AnsibleModule", return_value=check_module),
        patch.object(static_routes_module, "StaticRoutesFacts") as facts_mock,
        patch.object(static_routes_module, "load_config") as load_mock,
    ):
    ...
    assert check_module.exit_json.call_args.kwargs["commands"] == [
    ...
    with (
        patch.object(static_routes_module, "AnsibleModule", return_value=module),
        patch.object(static_routes_module, "StaticRoutesFacts") as facts_mock,
        patch.object(static_routes_module, "load_config") as load_mock,
    ):
        facts_mock.side_effect = [
            Mock(facts={"static_routes": existing}),
            Mock(facts={"static_routes": desired}),
        ]
        with pytest.raises(ExitJson):
            static_routes_module.main()
    ...
    load_mock.assert_called_once_with(
        module, ["ip route 192.168.2.0 255.255.255.0 10.0.0.1"]
    )
...
def test_static_routes_infers_ipv6_route_type_when_omitted():
    ...
    assert static_routes_module.route_key(route) == (
        "ipv6",
        "2001:db8::",
        "32",
        "2001:db8::1",
    ...
    with pytest.raises(
        static_routes_module.ReconciliationInputError, match="not admitted"
    ):
        static_routes_module.build_static_route_commands([route], [])
...
def test_static_routes_infers_ipv4_route_type_when_omitted():
    ...
    assert static_routes_module.build_static_route_commands([route], []) == [
...
def test_static_routes_rendered_does_not_gather_or_apply():
    ...
    assert module.exit_json.call_args.kwargs["changed"] is False
    assert module.exit_json.call_args.kwargs["rendered"] == [
        "ip route 192.0.2.0 255.255.255.0 192.0.2.1"
    ]
...
def test_static_routes_facts_failure_is_explicit():
    ...
    with (
        ...
        patch.object(
            ...
            "StaticRoutesFacts",
            side_effect=Exception("route gather failed"),
        ),
        pytest.raises(ExitJson),
    ):
        static_routes_module.main()
    ...
    assert (
        module.fail_json.call_args.kwargs["msg"]
...
def test_acls_lifecycle_uses_network_apply_and_check_mode_computes_diff():
    ...
    desired = [
        {
            "acl_id": 100,
            "acl_type": "standard",
            "rules": [{"action": "deny", "source": "any", "destination": "any"}],
        }
    ]

    check_module = _fake_module({"config": desired, "state": "merged"}, check_mode=True)
    with (
        patch.object(acls_module, "AnsibleModule", return_value=check_module),
        patch.object(acls_module, "AclsFacts") as facts_mock,
        patch.object(acls_module, "load_config") as load_mock,
    ):
    ...
    module = _fake_module({"config": desired, "state": "merged"})
    with (
        patch.object(acls_module, "AnsibleModule", return_value=module),
        patch.object(acls_module, "AclsFacts") as facts_mock,
        patch.object(acls_module, "load_config") as load_mock,
    ):
        facts_mock.side_effect = [
            Mock(facts={"acls": existing}),
            Mock(facts={"acls": desired}),
        ]
        with pytest.raises(ExitJson):
            acls_module.main()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "config": [
                    {"name": "ethernet 0/0/1", "description": "new", "enabled": True}
                ],
                "state": "merged",
            },
            {
        ...
        (
            ...
            {
                "config": [
                    {"name": "ethernet 0/0/1", "mode": "access", "access_vlan": 100}
                ],
                "state": "merged",
            },
                ...
                "config": [
                    {
                        ...
                        "ipv4": [
                            {"address": "10.0.0.2", "subnet_mask": "255.255.255.0"}
                        ],
                    }
                ],
                "state": "merged",
        ...
        (
            ...
            {
                "config": [
                    {"name": "eth-trunk 1", "mode": "static", "members": ["0/0/1"]}
                ],
                "state": "merged",
            },
...
def test_interface_family_lifecycle(
    module_under_test, facts_attr, facts_class, params, before, after, expected_commands
):
    ...
    with (
        patch.object(module_under_test, "AnsibleModule", return_value=unchanged),
        patch.object(module_under_test, facts_attr) as facts_mock,
        patch.object(module_under_test, "load_config") as load_mock,
    ):
    ...
    with (
        patch.object(module_under_test, "AnsibleModule", return_value=check_module),
        patch.object(module_under_test, facts_attr) as facts_mock,
        patch.object(module_under_test, "load_config") as load_mock,
    ):
    ...
    with (
        patch.object(module_under_test, "AnsibleModule", return_value=changed),
        patch.object(module_under_test, facts_attr) as facts_mock,
        patch.object(module_under_test, "load_config") as load_mock,
    ):
    ...
    with (
        patch.object(module_under_test, "AnsibleModule", return_value=failing),
        patch.object(
...
"""
    ...
    with (
        patch.object(ospf_v2_module, "AnsibleModule", return_value=module),
        patch.object(
            ospfv2_facts_module,
            "run_commands",
            return_value=[summary_output, neighbor_output],
        ),
    ):
        with pytest.raises(ExitJson):
            ospf_v2_module.main()
...
def test_xikeos_ospf_v2_surfaces_contextual_fact_gather_failures():
    ...
    with (
        ...
        patch.object(
            ...
            "run_commands",
            side_effect=xikeos_errors.XikeOSCommandError(
                "command execution failed", commands=["show ip ospf"], detail="lost"
            ),
        ),
    ):
        with pytest.raises(ExitJson):
...
@pytest.mark.parametrize(
    ...
    [
        (
            stp_module,
            {"config": {"stp_mode": "rstp"}, "state": "rendered"},
            {"config": {"stp_mode": "rstp"}, "state": "merged"},
        ),
        (
            erps_module,
            {"instance_id": 1, "state": "rendered"},
            {"instance_id": 1, "state": "present"},
        ),
        (
            eaps_module,
            {"domain_id": 1, "state": "rendered"},
            {"domain_id": 1, "state": "present"},
        ),
        (
            qinq_module,
            {"config": {"mode": "customer"}, "state": "rendered"},
            {"config": {"mode": "customer"}, "state": "merged"},
        ),
        (
            mirror_module,
            {
        ...
        (
            flex_monitor_link_module,
            {
                "config": {
                    "flex_links": [
                        {"group_id": 1, "master_port": {"type": "eth", "id": "0/0/1"}}
                    ]
                },
                "state": "rendered",
            },
...
def test_specialty_modules_are_rendered_only_or_fail_fast(
    module_under_test, rendered_params, mutating_params
):
    ...
    assert (
        rendered.exit_json.call_args.kwargs["rendered"]
        == rendered.exit_json.call_args.kwargs["commands"]
    )
